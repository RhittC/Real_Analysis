...
A good portion of the text is also completely original content that arose from research and \textit{pause and ponder}\cite{3b1b} about specific curiosities as I progressed through Tao.
...
The most obvious reason to do Analysis is personal: to understand deeper concepts that stray far from the base intuition you get from VCE and high school in general.
...
And you realise the true degree of freedom you actually have in maths. If you don't like standard definitions or formulations, you can just make your \textit{own} - full independence.

\subsubsection{Motivation 2 (very important)}

That's all well and good, but there's a much deeper problem underlying maths that \textit{demands} the level of rigour that is Analysis, as an absolute \textit{necessity}.
...
And they held this assumption to be so blatantly obvious that it underpinned their philosophy of everything from music to cosmic harmony\cite{Pythag}.
...
The truth is, we're \textit{not} sure. And it's in fact been \textit{proven}\cite{Goedel}\footnote{see Kurt G\"odel's Incompleteness Theorem 2 (on consistency)} that we we can never be sure. 
Not unless we rob ourselves of \textit{primary school arithmetic}, which would be very absurd.
So is maths condemned to be a Sisyphean task; will we have to rebuild maths from the ground up, like the Greeks, every other century?
No one can deny the possibility. So we do the best we can to minimise that risk: we lower our assumptions to the bare minimum that is still feasible, and \textit{rigorously} show that we make no more implicit assumptions.
...
\begin{wrapfigure}{l}{0.2\textwidth} \centering
    ...
    [insert dreaming dolphin figure here]
    ...
    .
\end{wrapfigure}
To be clear, the issue we mention isn't with logic itself. In fact, the question of whether logic is valid is philosophical in nature, and seemingly can't ever have a definitive resolution.
...
For instance, a mathematician needs to be able to say without a second thought that 
$$\text{if }P\text{ is }\tr\text{ and }P\text{ implies }Q\text{, then }Q\text{ is also }\tr$$
So they leave those deep questions about reality for the philosopher to concern themself with, and instead take the validity of logic to be given.

More specifically, the mathematician prescribes a list of objective and reasonable rules for how logic operates that they consider fundamental, independent of whatever content (in our case, maths) you choose to discuss with it.
Having set in stone this interface between logic and maths, the mathematician now gives themselves the freedom to question \textit{anything} in the realm of maths using proofs that adhere to the logical rules,
but they have so much confidence in those fundamental logical rules themselves that they never doubt them again.
This way, the mathematician is sure that everything they conclude, stated precisely, is as valid as logic itself, and they're content with that.
We will see an explicit formalism of logic next subsection.
...
It was soon proven with pure logic that the only such collections we can be sure have no contradictions are far too weak to be of much value to us \cite{Goedel}.
This will be explored in some more depth in Chapter $3$, on Hilbert's Program (consistency) \cite{Hilbert}.
...
substituting for where we would otherwise say $n \in \N$. We abbreviate ($\forall n\;s.t.\; \N[n]$) as ($\forall \N[n]$)
    ...
    \section{Local Maxima, Minima and Derivatives}
