...
And if either of those turn out to be False, then we have no information about whether (I am a monkey) is True or False. Thus, all in all, we have a ($\implies$) statement that is True but we can't show definitely that its implication that (I am a monkey) is also True\dots which is a good thing because I'm pretty confident I'm not a monkey (but can I \textit{show} definitively that I'm not? Once again, no). 

This is a necessary tradeoff we made to establish barebone logic the philosopher can't object to.

So how do we prove a statement $P$? 
Instead of the fool's errand of looking for whether $P$ is definitively True,
we loosen our standards slightly and instead show the truth of a ($\implies$) statement as follows: if we accept a base set of assumptions, which we call our \textit{axioms}, then the result we're trying to prove is True.
...
\begin{result}{Definition}{D}{The Gold Standard of Proof}{green}
    A conjecture $C$ is considered ``proven'' under an axiomatic system $A_1\&A_2\&A_3\&\dots$ \textit{if and only if} we can show the following under our logic:
