...
So they're for the philosopher to ponder over. Instead, the mathematician prescribes a handful of intuitive operations on True/False values, which formalise pure logic. E.g.
...
And if either of those turn out to be False, then we have no information about whether (I am a monkey) is True or False. I could very well be a monkey despite not being lazy. Thus, all in all, we have an ($\implies$) statement that is True but we can't show definitely that its implication that (I am a monkey) is also True\dots which is a good thing because I'm pretty confident I'm not a monkey (but can I \textit{show} definitively that I'm not? Once again, no). 

This is a necessary tradeoff we made to establish barebone logic the mathematician will never have to doubt.

So how do we prove a statement $P$ (called a conjecture until proof is established)? 
Instead of the fool's errand of looking for whether $P$ is definitively True,
we loosen our standards slightly and instead show the truth of an ($\implies$) statement as follows: if we accept a base few assumptions, which we call our \textit{axioms}, then $C$ is True.
That is, our standard of proof for $P$ is:
$$Axiom_1\;\&\;Axiom_2\;\& Axiom_3\;\&\;\dots\quad\implies\quad C$$
...
Nonetheless, mathematicians collectively agree on a handful of axioms on sets called $\ZF$ (all of which we will encounter in the next 2 chapters) as the basis for our gold standard of proof, 
...
This subchapter is mainly for reference in case we wish to clarify logical definitions.

Define a statement as a logical object that takes on either $\tr$ (True), $\fa$ (False) or $ \ill $(Ill-Formed),
where $\mathfrak{i}$ indicates the statement is left undefined and considered to have no truth value. 
...
Furthermore, an ($\implies$) statement only comments about \textit{correlation}, not \textit{causation}.
 So you could satisfy (I have a german shepherd $\implies$ roses are red) if both are true (or even if you don't have a german shepherd), even if there's no causal link between the two individual statements.
...
As mentioned, $\ZF$, the ultimate handful of axioms we are leading up to, lay out the what is legally a set. So what even is a set intuitively? Informally, a set $S$ is the collection of all objects $x$ satisfying a certain predicate\footnote{see 1.4 Logic Preliminaries} $P_S[x]$. That is,
$$ x \in S \iff P_S[x]$$
...
As sets have not yet been defined, we primitively let $\N[n]$ be the predicate for whether $x$ is natural, 
substituting for where we would otherwise say $n \in \N$. We abbreviate ($\forall n\;s.t\; \N[n]$) as ($\forall \N[n]$)
    ...
    And yet, informally speaking, this leads to a modulo cycle in which only $n+1$ naturals exist, and everything beyond that overflows back to $0$ as illustrated below, like data structures do in computers. 
    Modulo sets have many applications, but they are not what we desire as a full, infinite, set of ``counting numbers'', particularly if that $n$ turns out to be $0$ itself, leading to $0$ trivially being the only natural.
    ...
    An offset modulo cycle once again leads to finitely many naturals, which we don't want.
    ...
    Indeed, it is logically consistent if you consider it to be no more than a template for generating individual axioms for every predicate you need as you go, all of which are themselves logical.
    ...
    \begin{result}{Axiom Schema}{P}{(PMI) Principle of Mathematical Induction}{red}
        ...
        $(\forall P:\rightsquigarrow \{\text{True}, \text{False}\})\qquad(P[0]\;\;\;\&\;\;\;((\forall \N[k])\;P[k]\!\!\implies\!\!P[k\suc])) \quad \implies \quad (\forall \N[n])\; P[n]$
