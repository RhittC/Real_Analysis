...
\newcommand{\preddef}[1]{#1:\myforall\rightsquigarrow \;\sqsubset\!\! \tr, \fa, \ill\!\!\sqsupset}
...
\title{Tao Analysis - My Formulation, Visualisations and Proofs}
\author{Rhitt C}
\date{January 2026}
...
Predicates (not a standard term in the literature) are primitive analogues of functions. 
They can be thought of as like functions with domain being all mathematical objects and range being the truth values.
...
As mentioned, $\ZF$, the ultimate handful of axioms we are leading up to, lay out what is legally a set. So what even is a set intuitively? Informally, a set $S$ is the collection of all objects $x$ satisfying a certain predicate\footnote{see 1.4 Logic Preliminaries} $\preddef{P}$. That is,
$$ x \in S \iff \pred{P}{x}$$
...
substituting for where we would otherwise say $n \in \N$. We abbreviate ($\forall n\;s.t.\; \N[n]$) as ($\forall \N[n]$)
    ...
    $\preddef{P}$ is a predicate, i.e. a primitive version of a function whose input is any mathematical object (not possible with functions) and output is a truth value.
