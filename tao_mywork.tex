...
\usepackage{amsmath,amsthm,amssymb,hyperref, multicol, tikz, romannum, wrapfig, graphicx}
...
A sizeable portion of the text is also completely original content that arose from research and \textit{pause and ponder}\footnote{as suggested by Grant Sanderson, 3Blue1Brown} about specific curiosities as I progressed through Tao.
...
The truth is, we're \textit{not} sure. And it's in fact been \textit{proven}\footnote{see Kurt G\"odel's Incompleteness Theorems} that we we can never be sure. 
Not unless we rob ourselves of the ability to do primary school level arithmetic, which would be very absurd. So as we shall see, we do the next best thing: minimise our set of assumptions to the bare minimum that is still feasible.
...
For example, it could very well be the case that we and our fictitious universe only exist in the vivid yet incoherent dream of a celestial dolphin. If so, logic as we perceive it only \textit{appears} valid to us,
yet completely flawed and contradictory to a scientist observing the dream from outside in the real universe.
...
So they leave those deep questions for the philosopher to concern themself with, and instead take the validity of logic to be a given.

More specifically, the mathematician prescribes a series of objective and reasonable rules for how logic operates, independent of whatever content (in our case, maths) you choose to discuss with it, and hands it over for the philosopher to contest.
...
but never again to doubt the rules themselves.
This way, the mathematician is sure that everything they conclude is as valid as logic itself.
And if the philosopher someday finds that the true logical rules are indeed different in the real universe waiting beyond the dolphin's dream,
then they could still very well be similar enough for the bulk of the mathematician's work to be moulded accordingly.
...
But what matters for maths is, how do we prove a statement $P$ under our formal rules for logic? 
...
Of course, in perhaps the greatest failure of modern maths, we've shown logically that we still can't ever completely eliminate the risk, 
...
\subsubsection{Statements - Pure Logical Operations}
\subsubsection{$0^{th}$ Order Objects and $1^{st}$ Order Logic - Link with Maths}
\subsubsection{Restricted $2^{nd}$ Order Logic - Schema}
...
\subsection{Peano Axioms - Motivation for Defining $\N$ Axiomatically}
