...
For example, you may have heard the result that there are more $x\in\R$ with $x\in(0, 1)$ than there are $p\in\Q$ with $p\in(-\infty, \infty)$.
...
But in Real Analysis, we handle infinity \textit{rigorously} to make such statements not only meaningful, but to fully understand the motivation for even discussing such matters in the first place.

Also, it answers exactly what you can and can't do with infinity. E.g. Take the following example to illustrate why you need to satisfy some very special conditions to be able to interchanging the order in which you perform infinitely many operations without contradiction. 
Consider the matrix
$$ M = \begin{pmatrix}
    1   & 0     & 0     & 0     & \cdots \\
    -1  & 1     & 0     & 0     & \cdots \\
...
\end{pmatrix} $$
You'd expect the sum of all elements $\sum M_ij$ to be the same 
...
For example, take $\sqrt{2}$, a quantity that arises in geometry very naturally as the length of the hypotenuse triangle with both other side lengths $1$.
In ancient Greece, many held the worldview that everything in the real world, like the diagonal of this square, can be quantified as a rational number in $\Q$.
...
So when it was found that $\sqrt{2}\in\Q$ would imply a contradiction, even though the square is very much real, it led to a crisis in which many ancient Greeks had to reevaluate their entire philosophy.
...
The truth is, we're \textit{not} sure. And it's in fact been \textit{proven}\footnote{search up Godel's Incompleteness Theorem} that we we can never be sure. 
Not unless we rob ourselves of the ability to do primary school arithmetic, which would be very absurd.

To be clear, the issue isn't with logic itself. As an aside, the question of whether logic is valid is philosophical in nature, and seemingly can't ever have a definitive resolution.
...
doubting logic every step of the way certainly limits what mathematical progress is possible.
So they're for the philosopher to ponder over. Instead, the mathematician prescribes a handful of syntactic operations on True/False (called boolean) values. E.g.
\begin{align*} &\text{(True }\&\text{ True) is assigned True, but} \\ &\text{(True }\&\text{ False) is assigned False, because True and False are said to contradict each other.} \end{align*}
Then they prescribe a few more syntactic rules on how to construct a boolean value from a logical statement that performs certain operations on mathematical objects.
E.g.
\begin{align*} (\forall x, P(x)\implies P(0)\text{) is assigned True} \end{align*}
Finally, they combine all of this into what's very aptly named the formal grammar of the formal language called logic. Confident in this grammar, they hand it over to the philosopher to question the validity of.
The mathematician is now happy to accept this grammar as absolute and instead ponders questions about what statements follow from others (namely, the assumptions) in the language of logic. 
...
So what's the best we can do? First, we isolate what assumptions, termed axioms, we are both confident in and deem to be indispensable for our maths to be of value. 
Then we 

Intuition is absolutely essential for \textit{developing} a conjecture and motivating why it might be plausible. 
But once we are confident that our conjecture is indeed plausible, we need a gold standard of proof that makes it near impossible for a contradiction to pop up and 
shatter the foundations of mathematics  as it did for the Greeks.
